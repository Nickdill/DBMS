...
class CommandInterpreter {
    ...
    void createStatement() {
        ...
        String name = name();
        if (_input.nextIf("as")) {
            Table tmp = selectClause();
            _database.put(name, tmp);
        } else {
            Table table = tableDefinition();
    ...
    Condition condition(Table... tables) {
        String el1 = columnName();
        Condition c;
        int i = 0;
        if (tables[i].findColumn(el1) == -1 && tables.length > 1) {
            i++;
        }
        String el2 = _input.next();
        String el3;
        if (_input.nextIs(Tokenizer.LITERAL)) {
            el3 = literal();
            c = new Condition(new Column(el1, tables), el2, el3);
        } else {
            el3 = columnName();
            c = new Condition(new Column(el1, tables), el2,
                new Column(el3, tables));
        }
        return c;
    }
